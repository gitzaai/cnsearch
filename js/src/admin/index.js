...
class CnsearchAdminButtons extends Component {
  oninit(vnode) {
    super.oninit(vnode);
  ...
  refreshStatus() {
    ...
    app
      ...
      .catch((error) => {
        ...
        this.statusText =
          error.response?.body?.errors?.[0]?.detail || app.translator.trans('gitzaai-cnsearch.admin.buttons.status_disconnected');
        this.documentCount = null;
        this.sourceDiscussions = null;
        this.sourcePosts = null;
        this.lastSync = null;
...
app.initializers.add('gitzaai-cnsearch', () => {
  app.registry
    .for('gitzaai-cnsearch')
    .registerSetting({
    ...
    .registerSetting(
      () =>
        m('div.Form-group', [
          ...
          CnsearchAdminButtons.component(),
        ]),
      -10,
      'cnsearch.meili.actions'
    );
});
